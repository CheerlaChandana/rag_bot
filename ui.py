...
def setup_directories():
    """Set up working directories for Streamlit Cloud"""
    try:
        # Use app's root directory or temporary directory in the cloud
        base_dir = Path.cwd() / "streamlit_docs"  # Relative to app root
...
def check_disk_space(dirs):
    """Check available space in the cloud environment"""
    try:
        usage = shutil.disk_usage(str(dirs["base_dir"]))
...
""", unsafe_allow_html=True)
    ...
    st.markdown("Explore your documents with an interactive AI-powered chat interface!", unsafe_allow_html=True)
    ...
    if not groq_api_key:
        st.error("GROQ_API_KEY not set. Please configure it in Streamlit Cloud secrets.")
        return
    ...
    with st.container():
        ...
        with col1:
            ...
            uploaded_files = st.file_uploader("Upload PDFs, TXTs, or CSVs", 
                                           type=["pdf", "txt", "csv"], 
